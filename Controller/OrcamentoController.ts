...
async function obterSaldoPorMes(dataSelecionada: Date) {
    try {
        ...
        const valorDefinidoTotal = orcamentos.reduce((total, orcamento) => total + orcamento.valorDefinido, 0);
        const valorAtualTotal = orcamentos.reduce((total, orcamento) => total + orcamento.valorAtual, 0);
...
export { obterOrcamentosPorData, obterSaldoPorMes };